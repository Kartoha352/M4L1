...
class DatabaseManager:
    ...
    def get_rating(self): #возвращает таблицу (имя пользователя и количество призов, которые он получил). 
                        #   Данные в таблице отсортированы в порядке убывания. Максимум в списке должно оказаться 10 победеителей.
        conn = sqlite3.connect(self.database)
        with conn:
            cur = conn.cursor()
